"""
BOBYQA optimization job - Phase 1 Implementation
...
class BOBYQAJob(Job):
    ...
    def process_result(self, result):
        ...
        # --- Trust Region Step Evaluation ---
            ...
            if accepted:
                ...
                # Step rejected - still add to interpolation set to improve model
                x_trial = self.model_center + s
                self.interpolation_points.append((x_trial, result['target_val']))

                # Keep interpolation set bounded
                if len(self.interpolation_points) > self.n_model_points:
                    self.interpolation_points.pop(0)

                # Rebuild model with new information
                self._build_quadratic_model()

                # Don't count rejected steps toward no_improvement_count
                # Trust radius shrinking means we're still exploring
    ...
    def _check_convergence(self):
        ...
        # Use a more lenient threshold - BOBYQA needs more iterations than gradient methods
        ...
        if self.type == 'INITIAL_OPTIMIZATION':
            ...
            if grid_idx in self.sampler.population:
                ...
                if self.type == 'BOBYQA_LOOP':
                    ...
                    state['status'] = 'optimized'  # Final state
