...
| `de.smooth_certify`                | `False`                        | Opt-in: reuse neighbour (or cross-projection) argmax agreement to confirm convergence on a 1-generation window for smooth-interior cells (see below) |
...
**Smooth-certified fast convergence (`de.smooth_certify`, opt-in).** Every active grid cell normally spends at least `de.convergence_window` DE generations just *confirming* it has converged. When the parameters you profile out enter smoothly (e.g. Gaussian-constrained nuisances), the profiled argmax field is locally smooth and that confirmation budget is largely wasted on the ROI interior. With `de.smooth_certify=True`, a freshly activated cell confirms convergence on a **single** generation instead of the full window when it is certifiably on a smooth, single-valued patch — triggered either by its in-population neighbours agreeing on the profiled argmax, or (on later projections) by an earlier projection having already evaluated an ROI-competitive optimum essentially at this cell that agrees with the cell's current argmax. Both reuse information already in hand (neighbour params / the cross-projection solution pool) and the same activation-seed multimodality guard. The cell still runs DE, so the early exit is self-correcting — a cell that improves in that generation keeps evolving. The cross-projection trigger mainly reaches early/edge-of-front cells that lack the settled neighbours the first trigger needs, adding a small further saving (~1–2% on Himmelblau-4D / Rosenbrock-4D, Mann–Whitney *p* ≤ 0.05) at no extra evaluations and no quality cost. A replicate study (`examples/run_smooth_certify_replicate_study.py`, 6–8 seeds per mode, scoring ROI quality with a noise-robust deficit metric built from the one-sided lower-bound structure of profiling) settles when it helps. On targets whose profiled (inner) problem is unimodal it is a **robust win with no measurable quality cost**: Himmelblau-4D −13.7% target calls and Rosenbrock-4D −10.9% (both Mann–Whitney *p* < 0.01), with the ROI deficit statistically indistinguishable from baseline (*p* = 0.70 and 0.44) and full ROI coverage. It is **off by default** because on a genuinely multimodal *inner* problem (Rastrigin-4D) it causes a statistically significant regression: despite −29% calls, the ROI deficit rises (*p* = 0.001) and ROI coverage falls from 68% to 59% (*p* = 0.018), since one DE generation under-explores the competing inner modes. Enable it when the profiled-out dimensions enter smoothly (e.g. Gaussian-constrained nuisances), where the win is clean.
