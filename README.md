...
- `use_local_surrogate`: Enable two local-fast-approximation eval-skip paths (default: False). (1) DE prescreen: a small per-cell quadratic fit from cached neighbourhood samples is used to skip the worker evaluation of trial vectors confidently predicted to be worse than the slot occupant; multiple guards (R² gate, RMSE margin, value- and parameter-space extrapolation checks) fall back to a full evaluation whenever the local fit is unreliable. (2) Initial-optimization basin-redundancy skip: each global L-BFGS-B initial-optimization job is stopped (or skipped entirely at start) once its iterate enters a normalized 20%-of-bounds-diagonal radius around an already-found maximum, since further iterations would only refine a duplicate basin. Both paths are off-path-identical when the flag is False. Eval savings on the bundled benchmarks land in the low-single-digit percents — paraprof's initial-optimization phase is the dominant cost share, and the basin-skip recovers the bulk of what's reachable without changing the master/worker dispatch architecture.
