...
- `use_local_surrogate`: Enable local-quadratic surrogate-assisted DE trial pre-screening (default: False). DE fits a small per-cell quadratic from cached neighbourhood samples and skips full evaluation of trial vectors the surrogate confidently predicts will be worse than the current population member. Multiple guards (R² gate, RMSE margin, value- and parameter-space extrapolation checks) fall back to the off-path whenever the local fit is unreliable, so a poor surrogate never corrupts the ROI grid. Expected savings are small in paraprof's default config (single-digit percent) because initial-optimization evaluations — which run in the full parameter space the per-cell surrogate cannot help with — dominate the budget; the prescreen only affects the DE phase, where the eval-savings ceiling is bounded by the existing convergence-window detector.
