...
| `continuation.secant_predictor_warm_start`    | `True`             | When warm-starting an L-BFGS-B polish at a grid cell, additionally test linear/secant extrapolations of ψ\* along each grid axis built from two already-converged neighbors (first-order predictor). The best candidate becomes the L-BFGS-B starting point. Lifts grid quality on smooth targets at negligible cost; see `examples/run_continuation_benchmark_driver.py`. |
| `continuation.online_basin_switch`            | `False`            | As soon as an L-BFGS-B cell converges, check whether any already-converged neighbor has a profiled-param solution that beats this cell's, and immediately spawn a patching test/polish if so (rather than waiting for the post-hoc patching wave). Off by default: helps on truly multimodal targets but can spawn redundant polishes on smooth-valley targets. |
