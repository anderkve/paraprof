...
- `use_local_surrogate`: Enable local-quadratic surrogate-assisted DE trial pre-screening (default: False). When on, DE fits a small per-cell quadratic from cached neighbourhood samples and skips full evaluation of trial vectors the surrogate confidently predicts will be worse than the current population member. Safe to enable: multiple guards (R² gate, RMSE margin, in-support check) fall back to the off-path whenever the local fit is unreliable, so a poor surrogate never corrupts the ROI grid. Savings are modest on the bundled benchmarks because paraprof's default workflow distributes target calls across init / activation / L-BFGS-B polish; the prescreen only affects the DE phase.
