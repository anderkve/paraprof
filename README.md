...
The animation below replays a full two-projection scan of the 4D Himmelblau log-likelihood, sampled at regular intervals during a live MPI run. Each projection passes through the same stages: an **initial global L-BFGS-B sweep** (scattered red points) that locates promising starting maxima, then **dynamic grid activation** that grows the differential-evolution ROI outward cell by cell, and a final **patching wave** that polishes the result. Projection 1 (left, over `x₀, x₁`) and projection 2 (right, over `x₀, x₂`) profile out different pairs of the four parameters, and the two profiles end up with visibly different multi-peak structures — `(x₀, x₁)` inherits the four-peak 2D Himmelblau shape, while `(x₀, x₂)` separates into a sum of two 1D Himmelblau profiles and exposes the cartesian-product peak grid.
