"""
Render a README GIF for the 4D Himmelblau function with all six 2D
...
refinement -> patching waves -> suspect-cell recheck). Each subsequent
projection is warm-started from the accumulated global solution pool,
so its initial L-BFGS-B sweep is skipped and dynamic activation can
start immediately. The animation shows, in every frame, the live grid
for the projection currently being scanned, the frozen final grid for
projections that have already finished, and an inert grey placeholder
for projections still queued.

ParaProf and animation settings match ``make_readme_animation_rosenbrock.py``;
only the test function, ROI threshold (5.0 here) and the visible axis
ranges differ. The colour-bar runs from -5 to 0 (= -ROI threshold to 0).

Run with MPI:

    mpiexec -n 4 python examples/make_readme_animation_himmelblau_4D.py
...
# Coarser grid than the Rosenbrock animation (60x60 vs 100x100):
# Himmelblau-4D's cartesian-product projections (e.g. (x0, x2)) have
# 16 high-likelihood peaks vs 4 for the 2D-only projections, so the
# ROI cell count and total target-function call count per projection
# are several times larger than for Rosenbrock at the same grid
# resolution. 60x60 keeps the total run length, in-memory snapshot
# accumulation and final GIF size all comfortably bounded.
GRID_PER_DIM = 60
N_INITIAL_OPT = 20
ROI_THRESHOLD = 5.0
# ParaProf library defaults; matches the Rosenbrock animation. The
# suspect-cell recheck pass on the main branch (PR #13) cleans up any
# contiguous wrong-optimum strips, so the DE/LBFGSB knobs do not need
# to be bumped above their library defaults.
POP_PER_CELL = 3
LBFGSB_ITER = 50
...
GIF_PATH = os.path.join(OUT_DIR, "paraprof_himmelblau_4D.gif")
...
# Himmelblau peaks live). The four Himmelblau minima span roughly
# x_i in [-3.78, 3.58], so [-5, 5] gives a clean symmetric framing.
PANEL_XLIM = (-5.0, 5.0)
PANEL_YLIM = (-5.0, 5.0)
...
def _draw_panel(ax, axes_x, axes_y, grid_img, mask,
    ...
    # Crop the visible view to where the Himmelblau peaks live (data
